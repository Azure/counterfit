...
class DigitKeras(CFTarget):
    ...
    def load(self):
        if not os.path.isfile(self.fullpath(self.endpoint)):
            ...
            self.model = K.models.load_model(self.fullpath(self.endpoint))
    ...
    def create_model(self):
        # 0. get started
        print(f"    - Training new model. Begin MNIST using Keras {K.__version__}")
        np.random.seed(1)
        ...
        self.test_x = test_x
        # 2. define model
        print("   - Creating network with two Convolution, two Dropout, two Dense layers ")
        g_init = K.initializers.glorot_uniform(seed=1)
        opt = K.optimizers.Adam(learning_rate=0.01)
        x = K.layers.Input(shape=(28, 28, 1))
        con1 = K.layers.Conv2D(filters=32, kernel_size=(3, 3), kernel_initializer=g_init, activation='relu', padding='valid')(x)
        con2 = K.layers.Conv2D(filters=64, kernel_size=(3, 3), kernel_initializer=g_init, activation='relu', padding='valid')(con1)
        mp1 = K.layers.MaxPooling2D(pool_size=(2, 2))(con2)
        do1 = K.layers.Dropout(0.25)(mp1)
        z = K.layers.Flatten()(do1)
        fc1 = K.layers.Dense(units=128, kernel_initializer=g_init, activation='relu')(z)
        do2 = K.layers.Dropout(0.5)(fc1)
        fc2 = K.layers.Dense(units=10, kernel_initializer=g_init, activation='softmax')(do2)
        model = K.models.Model(x, fc2)
        model.compile(loss='categorical_crossentropy', optimizer=opt, metrics=['accuracy'], run_eagerly=False)
        # 3. train model
        batch_size = 100
        max_epochs = 2
        print(f"    - Starting training with batch size {batch_size}")
        model.fit(train_x, train_y, batch_size=batch_size, epochs=max_epochs, verbose=1)
