...
parser.add_argument("-a", "--attack", default=None, type=str, nargs='+', choices=CFState.get_instance().loaded_attacks.keys(), help="choose a specific set of attacks (default: all applicable)")
...
def parse_sample_index_param(set_params):
    # set random attack sample only if user hasn't set in the parameters. Rteurns sample_index as list or int type
    sample_index_list = re.findall(r"(\w+)\s?=\s?([\w\.]+)", " ".join(set_params))
    sample_index_list.extend(re.findall(r"(\w+)\s?=\s?(`.*`)", " ".join(set_params)))
    if sample_index_list:
        sample_index = sample_index_list[0][1]
        try:
            sample_index = int(sample_index)
        except:
            sample_index = utils.parse_special_string_w_eval(sample_index)
    else:
...
def perform_attacks_on_target(self, attacks, num_iters, has_logging_enabled, has_verbose_enabled, set_params, class_summary):
    sample_index = parse_sample_index_param(set_params)
    ...
    for attack_name in attacks:
        for n in range(num_iters):
            ...
            CFState.get_instance().active_target.set_attack_samples(sample_index)
            
            # Initialize the attack to obtain initial labels
